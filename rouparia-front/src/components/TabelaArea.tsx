...
export default function TabelaAreas({ areas, tipo }: TabelaAreasProps) {
  ...
  const onExcluirArea = async (id: number) => {
    if (confirm("Tem certeza que deseja excluir esta área?")) {
      try {
        ...
        alert(
          `Erro ao excluir a área: ${error.response?.data?.message || error.message
          }`
        );
      }
    }
  ...
  const onExcluirVinculo = async (id: number) => {
    if (confirm("Tem certeza que deseja excluir este vínculo?")) {
      try {
        ...
        alert(
          `Erro ao excluir a vínculo: ${error.response?.data?.message || error.message
          }`
        );
      }
    }
  ...
  if (!areas || areas.length === 0) {
    return (
      <>
        ...
        <button
          onClick={() => onCadastrar()}
          className="text-verde-primario flex flex-row hover:text-verde-primario-hover cursor-pointer"
          title="Adicionar"
        >
          Não há cadastros 
          <PlusIcon size={20} />
  ...
  return (
    <>
      {(isModalOpen && operacao) && <PopUpCadastrarArea setIsModalOpen={setIsModalOpen} operacao={operacao} areaVinculo={areaVinculo} />}
      <table className=" w-[80vw] overflow-y-hidden">
        <thead className="text-left sticky top-[10vh] xl:top-[9.2vh]">
          <tr className="bg-[#F2F2F2] shadow-md rounded-l-xl rounded-r-xl">
            <th className="px-4 py-2 font-poppins-semiBold">{tipo === 'area' ? 'Nome da Área' : 'Nome do Vínculo'}</th>
